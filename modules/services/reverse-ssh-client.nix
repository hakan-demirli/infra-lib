      ...
      assertions = [
        {
          assertion =
            ...
            || (
              ...
              && cfg.remotePort == 0
              && cfg.remoteUser == ""
              && cfg.localTargetPort == 22
              && cfg.localTargetHost == "localhost"
              && cfg.privateKeyPath == ""
              && cfg.sessionName == "reverse-tunnel"
              && cfg.remoteBindAddress == "localhost"
            );
          message = "services.reverse-ssh-client payload is configured while enable=false.";
        }
        {
          assertion =
            !cfg.enable
            || (
              cfg.username != "REVERSE_SSH_USER_UNSET"
              && cfg.remoteHost != ""
              && cfg.remotePort > 0
              && cfg.remotePort <= 65535
              && cfg.remoteUser != ""
              && cfg.localTargetPort > 0
              && cfg.localTargetPort <= 65535
              && cfg.privateKeyPath != ""
            );
          message = "services.reverse-ssh-client requires inventory username, remote host/user/port, local port, and private key when enabled.";
        }
      ];
    }
