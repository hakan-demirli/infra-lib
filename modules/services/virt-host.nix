...
let
  ...
  mkDomainXml =
    ...
    let
      ...
      cpus = toString g.virt.cpus;
      ramKb = toString (1048576 * g.virt.ram_gb);
    in
  ...
  mkGuestProvisionScript = g: ''
    ...
    baseImg="$pool/base/${g.virt.image}.${virt.images.${g.virt.image}.format}"

    if [ ! -f "$disk" ]; then
      ${pkgs.qemu_kvm}/bin/qemu-img create -f qcow2 \
        -F ${virt.images.${g.virt.image}.format} -b "$baseImg" \
        "$disk" ${toString g.virt.disk_gb}G
...
in
{
  config = lib.mkIf active {
    assertions = [
      {
        assertion = virt.role == "host" && virt.pool_path != null && virt.bridge != null;
        message = "virt-host requires virt.role=host with non-null pool_path and bridge.";
      }
    ];
    ...
    systemd = {
      ...
      services."virt-host-base-images" =
        let
      ...
      services."virt-host-guest-provisioning" = lib.mkIf (guests != [ ]) {
