...
let
  ...
  tierFor = tid: cluster.accessTiers.${tid} or null;
  ...
  effectiveTier =
    userGrants:
    let
      tiers = filter (t: t != null) (map (g: tierFor g.tier) userGrants);
  ...
  mkUserEntry =
    ...
    let
      ...
      homeDir = if sa.home == "/home/PLACEHOLDER" then "/home/${sa.username}" else sa.home;
    in
  ...
  rootAuthorizedKeys = unique (
    concatLists (
      map (
        ...
        in
        if u.is_root_anywhere && allowedOnThisHost u then u.keys.ssh else [ ]
      ) granted
    )
...
{
  options = {
    users.withSops = mkOption {
      type = types.bool;
      default = true;
    };
    users.deletedUsers = mkOption {
    ...
    users.users = mkOption {
      type = types.attrsOf (
        types.submodule (_: {
          options.allowedHosts = mkOption {
            type = types.listOf types.str;
            default = [ "all" ];
          };
          options.cohort = mkOption {
            type = types.enum [
              "admin"
              "staff"
              "student"
