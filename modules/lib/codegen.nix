...
let
  ...
  admins = filterAttrs (_: u: u.is_root_anywhere) users;
  ...
  sopsYaml =
    ...
    let
      ...
      yaml = builtins.toJSON {
        creation_rules = [ cluster ] ++ perCluster ++ perHost ++ perRole;
      };
    in
  ...
  headscaleAcl =
    ...
    let
      ...
      adminRule = {
        action = "accept";
        src = [ "group:admin" ];
        dst = map (cid: "${broadTagOf cid}:*") (attrNames activeClusters);
      };
