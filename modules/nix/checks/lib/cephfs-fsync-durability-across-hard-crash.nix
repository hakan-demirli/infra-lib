...
pkgs.testers.runNixOSTest {
  name = "cephfs-fsync-durability-across-hard-crash";
  ...
  testScript = ''
    ...
    with subtest("durable.txt is visible from client_2 BEFORE crash"):
        v = client_2.wait_until_succeeds(
            "cat /mnt/ceph/durable.txt"
        ).strip()
        assert v == "durable-data-1", f"pre-crash propagation failed: {v!r}"
    ...
    with subtest("durable.txt still readable after the crash"):
        v = client_2.wait_until_succeeds(
            "cat /mnt/ceph/durable.txt", timeout=60
        ).strip()
        assert v == "durable-data-1", (
            f"DURABILITY VIOLATION: client_2 lost durable.txt after "
            f"crash on storage_c. Got: {v!r}. "
            f"This means a successful fsync() did NOT actually persist "
            f"data to the surviving OSDs."
        )
        print("INVARIANT HELD: fsync'd data survives hard crash of "
              "one storage node (durability across replication holds)")
  '';
}
