{
  config,
  inputs,
  pkgs,
  lib,
  host ? null,
  ...
}:
let
  cfg = config.services.headscale-server;
  aclPkg = inputs.self.lib.codegen.headscaleAcl { inherit pkgs; };
  impermanenceEnabled = host != null && (host.impermanence.enable or false);
in
{
  options.services.headscale-server = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable Headscale control server with Caddy";
    };
    serverUrl = lib.mkOption {
      type = lib.types.str;
      default = "";
      description = "Public URL for Headscale (e.g., sshr.example.com)";
    };
    allowedUDPPorts = lib.mkOption {
      type = lib.types.listOf lib.types.int;
      default = [
        3478
        41641
      ];
      description = "UDP ports to allow for STUN/Tailscale discovery";
    };
  };

  config = lib.mkMerge [
    {
      assertions = [
        {
          assertion =
            cfg.enable
            || (
              cfg.serverUrl == ""
              &&
                cfg.allowedUDPPorts == [
                  3478
                  41641
                ]
            );
          message = "services.headscale-server payload is configured while enable=false.";
        }
        {
          assertion = !cfg.enable || cfg.serverUrl != "";
          message = "services.headscale-server.serverUrl must be set when Headscale is enabled";
        }
      ];
    }

    (lib.mkIf cfg.enable {
      services = {
        headscale = {
          enable = true;
          settings = {
            server_url = "https://${cfg.serverUrl}";
            listen_addr = "0.0.0.0:8080";
            ip_prefixes = [ "100.64.0.0/10" ];

            policy.path = "${aclPkg}/policy.hujson";

            dns = {
              magic_dns = true;
              base_domain = "ts.${cfg.serverUrl}";
              nameservers = {
                global = [
                  "1.1.1.1"
                  "8.8.8.8"
                ];
              };
            };

            derp = {
              server = {
                enabled = true;
                region_id = 999;
                region_code = "oracle-vps";
                region_name = "Oracle Cloud VPS";
                private_key_path = "/var/lib/headscale/derp_server.key";
                stun_listen_addr = "0.0.0.0:3478";
                verify_clients = true;
              };
              urls = [ ];
              auto_update_enabled = true;
              update_frequency = "24h";
            };
          };
        };

        caddy = {
          enable = true;
          virtualHosts."${cfg.serverUrl}" = {
            extraConfig = ''
              reverse_proxy * http://127.0.0.1:8080
            '';
          };
        };

        tailscale.extraUpFlags = [
          "--login-server=https://${cfg.serverUrl}"
          "--advertise-exit-node"
        ];
      };

      environment.persistence = lib.mkIf impermanenceEnabled {
        "/persist".directories = [
          "/var/lib/headscale"
          "/var/lib/caddy"
        ];
      };

      networking.firewall = {
        enable = true;
        inherit (cfg) allowedUDPPorts;
      };

      boot.kernel.sysctl = {
        "net.ipv4.ip_forward" = 1;
        "net.ipv6.conf.all.forwarding" = 1;
      };
    })
  ];
}
